...
<h1>Permissões - Atualizar</h1>

<form method="post" class="form">

	<label for="name" class="nome_titulo">Nome da Permissão</label>
	<input type="text" name="name" id="name" placeholder="Nome da Permissão" autofocus="autofocus" class="form-control" required="required" value="<?php echo $dadosperimissao['name_apresentar']; ?>" ><br>

	<input type="text" name="descricion" id="descricion" placeholder="Descrição da Permissão" autofocus="autofocus" class="form-control" required="required"  style="pointer-events: none;background: #ccc;" value="<?php echo $dadosperimissao['name']; ?>" ><br>

	<select name="public_private" id="public_private" class="form-control public_private" required="required" >
		<option value="0"  <?php echo($dadosperimissao['public_private'] == 0)? 'selected' : ''; ?>  >Publica</option>
		<option value="1"  <?php echo($dadosperimissao['public_private'] == 1)? 'selected' : ''; ?> >Privada</option>
	</select><br>
	...
	<select name="lista_pages" id="lista_pages" class="form-control lista_pages" required="required" >
		...
		<?php foreach($lista_pages as $li): ?>
			...
			<option value="<?php echo $li['id']; ?>"  <?php echo($dadosperimissao['page_reference'] == $li['id'])? 'selected' : ''; ?> ><?php echo $li['name']; ?></option>
